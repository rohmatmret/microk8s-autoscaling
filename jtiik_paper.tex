...
Zhang et al. \cite{zhang2021deep} menyediakan survei komprehensif tentang deep RL untuk alokasi sumber daya cloud, mengidentifikasi tantangan kunci termasuk efisiensi sample, keseimbangan exploration-exploitation, dan optimasi multi-objektif. Penelitian terkini oleh Santos et al. \cite{santos2024efficient} mengeksplorasi deployment multi-cluster Kubernetes menggunakan RL, sementara Mampage et al. \cite{mampage2023deep} fokus pada serverless autoscaling dengan optimasi waktu dan biaya.

\textbf{Perkembangan Terkini (2024-2025):} Islam et al. \cite{islam2021performance} mendemonstrasikan deep RL untuk Spark job scheduling dengan trade-off performa-biaya. Barua dan Kaiser \cite{barua2023ai} mengeksplorasi framework AI-driven untuk platform hybrid cloud. Kemajuan state-of-the-art telah memperluas kapabilitas autoscaling berbasis RL: KARMA (Kubernetes Adaptive Resource Management Architecture) memperkenalkan resilience-aware autoscaling yang mempertahankan performa di bawah node failures dan network partitions melalui multi-objective RL dengan explicit failure modeling. DRPC (Distributed Reinforcement-based Policy Coordinator) mengatasi tantangan distributed scalability lintas federated microservices architectures, memungkinkan coordinated scaling decisions lintas service mesh boundaries. KIS-S (Kubernetes Intelligent Simulator with GPU awareness) menyediakan GPU-aware simulation environments untuk training autoscaling policies yang mengoptimalkan heterogeneous compute resources (CPU/GPU/TPU), khususnya relevan untuk machine learning workloads. Perkembangan ini mendemonstrasikan lanskap RL-based cloud resource management yang semakin matang, meskipun sebagian besar masih dalam fase penelitian tanpa widespread production adoption.
...
\toprule
\textbf{Penelitian} & \textbf{Algoritma} & \textbf{Multi-Obj.} & \textbf{SLA-Aware} & \textbf{Fair Comp.} & \textbf{Impl. Guide} \\
\midrule
Mao et al. \cite{mao2016resource} & DQN & ✗ & ✗ & ✗ & ✗ \\
Xu \& Buyya \cite{xu2021adaptive} & PPO & ✗ & ✓ & ✗ & ✗ \\
Santos et al. \cite{santos2024efficient} & DQN & ✗ & ✓ & ✗ & ✗ \\
Mampage et al. \cite{mampage2023deep} & DRL & ✓ & ✗ & ✗ & ✗ \\
KARMA \cite{sharma2024karma} & Multi-obj. RL & ✓ & ✓ & ✗ & ✗ \\
\midrule
\textbf{Penelitian ini} & \textbf{Hybrid DQN-PPO} & ✓ & ✓ & ✓ & ✓ \\
\bottomrule
\end{tabular}
\vspace{2mm}
\small
\textit{Catatan:} Multi-Obj. = Multi-objective optimization, Fair Comp. = Fair comparison (konfigurasi identik), Impl. Guide = Implementation guide praktis
\end{table}

Tabel \ref{tab:related_work_comparison} menunjukkan keunggulan penelitian ini dalam menyediakan solusi komprehensif yang menggabungkan optimasi multi-objektif, kesadaran SLA, perbandingan yang adil, dan panduan implementasi lengkap—aspek yang kurang dalam penelitian sebelumnya.
...
\begin{algorithm}[H]
\caption{Pelatihan Hybrid DQN-PPO}
\begin{algorithmic}[1]
\STATE Inisialisasi DQN network $Q(s, a; \theta_{\text{DQN}})$ dan target network $Q(s, a; \theta^-)$
\STATE Inisialisasi PPO actor-critic $\pi_\theta(a|s)$, $V_\phi(s)$
\STATE Inisialisasi replay buffer $\mathcal{D}$
\FOR{episode $= 1$ to $N$}
    \STATE Reset environment ke state awal $s_0$
    \FOR{step $t = 0$ to $T$}
        \STATE Pilih action: $a_t = \arg\max_a Q(s_t, a; \theta_{\text{DQN}})$ dengan $\epsilon$-greedy
        \STATE Eksekusi action $a_t$, observasi $r_t, s_{t+1}$
        \STATE Hitung base reward: $r_{\text{base}} = R(s_t, a_t, s_{t+1})$
        \STATE Hitung PPO modulation: $m_{\text{PPO}} = \pi_\theta(a_t | s_t)$
        \STATE Hitung optimized reward: $r_{\text{opt}} = r_{\text{base}} \cdot (1 + 0.3 \cdot m_{\text{PPO}})$
        \STATE Hitung blended reward: $r_{\text{train}} = 0.7 \cdot r_{\text{base}} + 0.3 \cdot r_{\text{opt}}$
        \STATE Simpan transition $(s_t, a_t, r_{\text{train}}, s_{t+1})$ di $\mathcal{D}$
        \IF{$|\mathcal{D}| \geq$ batch\_size}
            \STATE Sample minibatch dari $\mathcal{D}$
            \STATE Update DQN: $\theta_{\text{DQN}} \leftarrow \theta_{\text{DQN}} - \alpha \nabla_{\theta_{\text{DQN}}} L_{\text{DQN}}$
        \ENDIF
        \IF{$t \mod$ update\_frequency $= 0$}
            \STATE Update target network: $\theta^- \leftarrow \theta_{\text{DQN}}$
        \ENDIF
    \ENDFOR
...
Strategi blending ini menyediakan sinyal pembelajaran stabil untuk DQN sambil memasukkan optimasi adaptif PPO.
...
\subsection{Overhead Komputasi dan Waktu Training}

Framework hybrid DQN-PPO memperkenalkan biaya komputasi tambahan dibanding HPA tradisional, yang perlu dipertimbangkan untuk deployment praktis:

\textbf{Kompleksitas Model:}
Arsitektur gabungan terdiri dari 9.284 trainable parameters (DQN: 4.675 parameter untuk Q-network + target network; PPO: 4.609 parameter untuk actor-critic). Total ukuran model sekitar 36,4 KB, memungkinkan deployment pada environment resource-constrained seperti edge computing atau single-node clusters.

\textbf{Kebutuhan Komputasi Training:}
Training dilakukan pada sistem berbasis CPU standar (Intel i7, 16GB RAM) tanpa akselerasi GPU. Proses training 50.000 langkah (dengan flag --mock untuk mode simulasi) selesai dalam waktu sekitar 28 menit wall-clock time, rata-rata 1.785 langkah per menit. Konsumsi memori tetap di bawah 2GB sepanjang training, termasuk penyimpanan replay buffer (140.978 samples). Hyperparameter optimization via Optuna memerlukan 20 trials per agent, total sekitar 18,7 jam untuk complete optimization (9,3 jam per agent).

\textbf{Latensi Inference:}
Selama evaluasi, latensi keputusan hybrid agent rata-rata 3,2ms per keputusan scaling (DQN forward pass: 1,8ms, PPO forward pass: 1,4ms) pada CPU. Ini negligible dibanding interval pengumpulan metrik HPA Kubernetes yang 15-30 detik, menambahkan overhead kurang dari 0,02\% ke control loop autoscaling. Akselerasi GPU (NVIDIA RTX 3080) mengurangi inference latency ke 0,4ms, meskipun tidak diperlukan mengingat keputusan scaling yang jarang (setiap 30 detik).

\textbf{Overhead Deployment Produksi:}
Pada environment produksi, trained model menjalankan inference-only tanpa komputasi gradient, memerlukan resource minimal. Service Python lightweight (Flask/FastAPI) yang hosting model mengkonsumsi sekitar 150MB RAM dan 0,1 vCPU, sebanding dengan custom metrics adapters yang sudah di-deploy di cluster Kubernetes produksi. Model dapat di-package sebagai sidecar container (5MB Docker image) atau diintegrasikan ke existing autoscaling controllers.

\textbf{Perbandingan dengan HPA Tradisional:}
Meskipun HPA memiliki computational overhead efektif nol (arithmetic sederhana pada observed metrics), overhead hybrid agent tetap negligible relatif terhadap konsumsi resource cluster-wide. Untuk cluster 100-node yang mengelola ribuan pods, tambahan 0,1 vCPU untuk RL-based autoscaling merepresentasikan kurang dari 0,01\% overhead sambil memberikan peningkatan performa terukur (7\% pengurangan SLA, 4,5\% penghematan biaya).
...
\subsubsection{Analisis Perilaku Temporal}

Untuk mengilustrasikan sifat proaktif dari agen hybrid DQN-PPO, kami menyajikan analisis time-series pada skenario lonjakan tiba-tiba, yang paling baik mendemonstrasikan perbedaan antara strategi autoscaling reaktif dan proaktif.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{sudden_spike_timeseries.png}
\caption{Perbandingan time-series pada skenario lonjakan tiba-tiba mengungkapkan perbedaan perilaku kritis. \textbf{Panel atas}: Incoming request rate (RPS) dengan peningkatan step-function trafik dari 2.000 ke 10.000 RPS pada langkah 150. \textbf{Panel tengah}: Agen hybrid mengantisipasi lonjakan trafik 2-3 langkah (60-90 detik) lebih awal, melakukan scaling pod secara proaktif berdasarkan pola yang dipelajari, sementara HPA hanya bereaksi setelah ambang CPU terlampaui, memerlukan 15 langkah untuk mencapai kapasitas target versus 8 langkah hybrid (47\% lebih cepat). \textbf{Panel bawah}: Latensi respons dengan threshold SLA 200ms ditandai. Scaling proaktif menjaga latensi hybrid di bawah 150ms sepanjang transisi, sedangkan HPA menunjukkan spike hingga 280ms selama delay scale-up, menghasilkan 54\% lebih banyak pelanggaran SLA selama window kritis 5 menit ini.}
\label{fig:sudden_spike_timeseries}
\end{figure}

Gambar \ref{fig:sudden_spike_timeseries} mengungkapkan perbedaan perilaku kritis:

\begin{enumerate}
    \item \textbf{Scaling Proaktif}: Agen hybrid mulai scaling up pods sekitar 2-3 langkah sebelum peningkatan trafik besar, dipelajari dari pola historis dalam distribusi trafik.

    \item \textbf{Stabilitas Latensi}: Dengan mempertahankan kapasitas yang cukup di depan demand, agen hybrid menjaga response latency di bawah 150ms bahkan selama transisi trafik, sementara HPA menunjukkan latency spikes hingga 280ms selama delay scale-up.

    \item \textbf{Penyesuaian Pod Lebih Halus}: Agen hybrid melakukan penyesuaian pod count bertahap (±1-2 pods per langkah) dibanding respons threshold binary HPA, mengurangi overhead scaling dan ketidakstabilan sistem.

    \item \textbf{Respons Lebih Cepat terhadap Spike}: Ketika trafik melompat dari 2.000 RPS ke 10.000 RPS pada langkah 150, agen hybrid mencapai kapasitas target dalam 8 langkah versus 15 langkah HPA, mengurangi pelanggaran SLA kumulatif sebesar 54\% selama periode kritis ini.
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{cpu_utilization_comparison.png}
\caption{Pola utilisasi CPU pada skenario lonjakan tiba-tiba mendemonstrasikan manfaat stabilitas dari kebijakan yang dipelajari. Agen hybrid mempertahankan utilisasi lebih dekat ke target 70\% melalui manajemen kapasitas proaktif (standard deviation: 12,3\%), sedangkan HPA menunjukkan osilasi parah antara 40\% (over-provisioned selama overshoot scale-up) dan 95\% (under-provisioned selama delay reaktif), dengan standard deviation 23,7\%—hampir dua kali lipat variabilitas hybrid. Penggunaan sumber daya yang stabil langsung diterjemahkan ke lebih sedikit pelanggaran SLA (mengurangi under-provisioning), biaya lebih rendah (mengurangi over-provisioning), dan overhead pod churn yang menurun. Kedua agen rata-rata utilisasi CPU serupa (52,6\% vs 52,1\%), mengkonfirmasi bahwa peningkatan stabilitas berasal dari anticipatory scaling bukan target resource berbeda.}
\label{fig:cpu_utilization}
\end{figure}

Gambar \ref{fig:cpu_utilization} mendemonstrasikan perbedaan efisiensi CPU. Standard deviation agen hybrid dalam utilisasi CPU adalah 12,3\%, dibanding 23,7\% HPA, mengindikasikan penggunaan sumber daya yang lebih stabil. Stabilitas ini langsung diterjemahkan ke pengurangan pelanggaran SLA (lebih sedikit under-provisioned states) dan biaya lebih rendah (lebih sedikit over-provisioned states).

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{reward_optimization_comparison.png}
\caption{Perbandingan optimasi reward menunjukkan konvergensi pembelajaran hybrid DQN-PPO. Kurva menunjukkan peningkatan reward kumulatif seiring waktu pelatihan, memvalidasi efektivitas arsitektur hybrid dalam menyeimbangkan eksplorasi dan eksploitasi.}
